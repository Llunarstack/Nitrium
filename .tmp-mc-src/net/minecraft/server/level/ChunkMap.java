...
public class ChunkMap extends SimpleRegionStorage implements ChunkHolder.PlayerProvider, GeneratingChunkMap {
	...
	public ChunkMap(
		...
		super(
			...
			bl,
			DataFixTypes.CHUNK,
			LegacyStructureDataHandler.getLegacyTagFixer(serverLevel.dimension(), supplier, dataFixer)
		);
		...
		this.distanceManager = new ChunkMap.DistanceManager(ticketStorage, executor, blockableEventLoop);
	...
	private void processUnloads(BooleanSupplier booleanSupplier) {
		...
		Runnable runnable;
		while ((i > 0 || booleanSupplier.getAsBoolean()) && (runnable = this.unloadQueue.poll()) != null) {
			i--;
			runnable.run();
		}
	...
	public CompletableFuture<ChunkResult<LevelChunk>> prepareTickingChunk(ChunkHolder chunkHolder) {
		...
		return completableFuture.thenApplyAsync(chunkResult -> chunkResult.map(list -> {
			...
			if (completableFuturex.isDone()) {
				...
				completableFuturex.thenAcceptAsync(object -> this.onChunkReadyToSend(chunkHolder, levelChunk), this.mainThreadExecutor);
			}
	...
	void dumpChunks(Writer writer) throws IOException {
		...
		for (Entry<ChunkHolder> entry : this.visibleChunkMap.long2ObjectEntrySet()) {
			...
			csvOutput.writeRow(
				...
				optional2.<Integer>map(levelChunk -> levelChunk.getBlockEntities().size()).orElse(0),
				this.ticketStorage.getTicketDebugString(l, true),
				this.distanceManager.getChunkLevel(l, true),
				optional2.<Integer>map(levelChunk -> levelChunk.getBlockTicks().count()).orElse(0),
				optional2.<Integer>map(levelChunk -> levelChunk.getFluidTicks().count()).orElse(0)
			);
		}
	}
	...
	private static String printFuture(CompletableFuture<ChunkResult<LevelChunk>> completableFuture) {
		try {
			ChunkResult<LevelChunk> chunkResult = completableFuture.getNow(null);
			if (chunkResult != null) {
	...
	private CompoundTag upgradeChunkTag(CompoundTag compoundTag) {
		return this.upgradeChunkTag(compoundTag, -1, getChunkDataFixContextTag(this.level.dimension(), this.generator().getTypeNameForDataFixer()));
	}

	public static CompoundTag getChunkDataFixContextTag(ResourceKey<Level> resourceKey, Optional<ResourceKey<MapCodec<? extends ChunkGenerator>>> optional) {
		CompoundTag compoundTag = new CompoundTag();
		compoundTag.putString("dimension", resourceKey.identifier().toString());
		optional.ifPresent(resourceKeyx -> compoundTag.putString("generator", resourceKeyx.identifier().toString()));
		return compoundTag;
	}
	...
	boolean anyPlayerCloseEnoughTo(BlockPos blockPos, int i) {
		...
		for (ServerPlayer serverPlayer : this.playerMap.getAllPlayers()) {
			if (this.playerIsCloseEnoughTo(serverPlayer, vec3, i)) {
				return true;
			}
		}
	...
	private boolean playerIsCloseEnoughForSpawning(ServerPlayer serverPlayer, ChunkPos chunkPos) {
		...
		double d = euclideanDistanceSquared(chunkPos, serverPlayer.position());
		return d < 16384.0;
	}
	...
	private boolean skipPlayer(ServerPlayer serverPlayer) {
		return serverPlayer.isSpectator() && !this.level.getGameRules().get(GameRules.SPECTATORS_GENERATE_CHUNKS);
	}
	...
	private void updateChunkTracking(ServerPlayer serverPlayer) {
		...
		if (!(
			serverPlayer.getChunkTrackingView() instanceof ChunkTrackingView.Positioned positioned
	...
	protected void tick() {
		...
		for (ChunkMap.TrackedEntity trackedEntity : this.entityMap.values()) {
			...
			if (bl || trackedEntity.entity.needsSync || this.distanceManager.inEntityTickingRange(sectionPos2.chunk().toLong())) {
	...
	public void forEachEntityTrackedBy(ServerPlayer serverPlayer, Consumer<Entity> consumer) {
		for (ChunkMap.TrackedEntity trackedEntity : this.entityMap.values()) {
			if (trackedEntity.seenBy.contains(serverPlayer.connection)) {
	...
	public void resendBiomesForChunks(List<ChunkAccess> list) {
		...
		for (ChunkAccess chunkAccess : list) {
			...
			for (ServerPlayer serverPlayer : this.getPlayers(chunkPos, false)) {
				map.computeIfAbsent(serverPlayer, serverPlayerx -> new ArrayList<>()).add(levelChunk2);
			}
		}
	...
	class DistanceManager extends net.minecraft.server.level.DistanceManager {
		protected DistanceManager(final TicketStorage ticketStorage, final Executor executor, final Executor executor2) {
	...
	class TrackedEntity implements ServerEntity.Synchronizer {
		...
		public void sendToTrackingPlayersFiltered(Packet<? super ClientGamePacketListener> packet, Predicate<ServerPlayer> predicate) {
			for (ServerPlayerConnection serverPlayerConnection : this.seenBy) {
				if (predicate.test(serverPlayerConnection.getPlayer())) {
					serverPlayerConnection.send(packet);
				}
			}
		}
