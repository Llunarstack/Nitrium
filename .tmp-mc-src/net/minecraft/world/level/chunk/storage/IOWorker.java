...
public class IOWorker implements ChunkScanAccess, AutoCloseable {
	...
	private boolean isOldChunk(CompoundTag compoundTag) {
		return compoundTag.getIntOr("DataVersion", 0) < 4295 ? true : compoundTag.getCompound("blending_data").isPresent();
	}
	...
	public CompletableFuture<Void> synchronize(boolean bl) {
		CompletableFuture<Void> completableFuture = this.<CompletableFuture<Void>>submitTask(
			...
			.thenCompose(Function.identity());
		return bl ? completableFuture.thenCompose(void_ -> this.submitThrowingTask(() -> {
			try {
				this.storage.flush();
				return null;
			} catch (Exception exception) {
